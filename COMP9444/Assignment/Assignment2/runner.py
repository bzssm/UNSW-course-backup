...
def load_glove_embeddings():
    ...
    if emmbed_file.is_file():
        ...
        # data = open("glove.6B.50d.txt", 'r', encoding="utf-8")  # python3
...
def train():
    ...
    print(np.array(glove_array).shape)


    training_data_text = load_data()
    training_data_embedded = embedd_data(training_data_text, glove_array, glove_dict)
    input_data, labels, dropout_keep_prob, optimizer, accuracy, loss, prob = imp.define_graph()
    ...
    for i in range(iterations):
        ...
        # print('batch data={}, label={}'.format(batch_data.shape, np.array(batch_labels).shape))
        sess.run(optimizer, {input_data: batch_data, labels: batch_labels,
                             dropout_keep_prob: 0.6})
        if (i % 50 == 0):
            loss_value, accuracy_value, summary, prob_temp = sess.run(
                [loss, accuracy, summary_op, prob],
                {input_data: batch_data,
                 labels: batch_labels})
            writer.add_summary(summary, i)
            print('\t'.join([
                "Iteration: ", str(i), "loss", str(loss_value),"acc", str(accuracy_value)]))
            # print(prob_temp[:20])
