...
def init(argv):
  ...
  if len(sys.argv) != 4:
    print('usage:', sys.argv[0], '[peer identifier] [successor #1 identifier] [successor #2 identifier]', file=sys.stderr)
    exit(1);
  ...
  for argNum in range(1, len(sys.argv)):
    ...
    if not (str.isdigit(sys.argv[argNum])) or not (0 <= int(sys.argv[argNum]) <= 255):
      print('error: provided identifier (' + sys.argv[argNum] +') in argument', argNum ,'was not an integer in [0,255] (inclusive).', file=sys.stderr)
      exit(1);
