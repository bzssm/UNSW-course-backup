...
Colours = enum(STATUS=1, WARNING=2, COMMAND=3, RED=4, GREEN=5, FILETRANSFER=6,
               CHURN=7)  # Colour identifiers for control code highlighting
...
def init(argv):
    ...
    if len(sys.argv) != 4:
        print('usage:', sys.argv[0], '[peer identifier] [successor #1 identifier] [successor #2 identifier]',
              file=sys.stderr)
        exit(1)
    ...
    for argNum in range(1, len(sys.argv)):
        ...
        if not (str.isdigit(sys.argv[argNum])) or not (0 <= int(sys.argv[argNum]) <= 255):
            print('error: provided identifier (' + sys.argv[argNum] + ') in argument', argNum,
                  'was not an integer in [0,255] (inclusive).', file=sys.stderr)
            exit(1)
...
def main(screen):
    ...
    consolePrint(screen, CONTROL.STATUS,
                 "Attempting to join the CDHT network as Peer (" + makeColComp(Colours.GREEN, str(myPeer)) + ")...")
    consolePrint(screen, CONTROL.STATUS,
                 "Successfully joined CDHT network.")  # simulate fake join message because it looks nice!
    consolePrint(screen, CONTROL.STATUS, "Welcome to this CDHT network!")
    consolePrint(screen, CONTROL.STATUS,
                 "Enter valid commands at the bottom of this terminal screen. Command " + makeColComp(Colours.COMMAND,
                                                                                                      "quit") + " will exit the application.")

    # Display warning message if screen width size is too small
    height, width = screen.getmaxyx()
    if (width < MIN_REC_WIDTH):
        consolePrint(screen, CONTROL.WARNING, makeColComp(Colours.RED, "A minimum terminal width of " + str(
            MIN_REC_WIDTH) + " characters is recommended (current: " + str(width) + ")."))
    ...
    while True:
        ...
        if s == "quit":
            ...
            consolePrint(screen, CONTROL.STATUS,
                         "Leaving CDHT network and terminating program. Please wait for running threads to terminate.")  # quit message
            ...
            try:
                ...
                consolePrint(screen, CONTROL.STATUS,
                             "Invalid command parameters were provided. Provided command was: " + s)
                continue
            ...
            try:
                ...
                consolePrint(screen, CONTROL.STATUS,
                             "Invalid file was requested. File name must be a 4 length numeral.")
                continue
            ...
            if fileStatus == FILECHECK.NOTAVAILABLE:
                ...
                consolePrint(screen, CONTROL.FTRES,
                             "File " + makeColComp(Colours.RED, reqFileHash) + " is stored locally.")
                continue
            ...
            consolePrint(screen, CONTROL.FTREQ, "File request message for " + makeColComp(Colours.RED,
                                                                                          reqFileHash) + " has been sent to successor Peer (" + makeColComp(
                Colours.GREEN, str(succ1)) + ").")
...
def pingMonitor(screen, Ping):
    ...
    while True:
        ...
        if succ1 != PEER.DEAD and succ1NumMissedAcks >= PING_MISSED_ACK_DEAD_NUM:
            consolePrint(screen, CONTROL.PEERCHURN,
                         "Peer (" + makeColComp(Colours.GREEN, str(succ1)) + ") is no longer alive.")
        ...
        if succ2 != PEER.DEAD and succ2NumMissedAcks >= PING_MISSED_ACK_DEAD_NUM:
            consolePrint(screen, CONTROL.PEERCHURN,
                         "Peer (" + makeColComp(Colours.GREEN, str(succ2)) + ") is no longer alive.")
        ...
        try:
            ...
            if msgType == Ping.REQ:
                consolePrint(screen, CONTROL.PINGREQ,
                             "A ping request message was received from Peer (" + makeColComp(Colours.GREEN,
                                                                                             str(senderPeerID)) + ")")
                ...
                if (pred1 != PEER.INVALID and pred2 != PEER.INVALID) and (
                        senderPeerID != pred1 and senderPeerID != pred2):
                    pred1 = PEER.INVALID
                    pred2 = PEER.INVALID
                ...
                consolePrint(screen, CONTROL.PINGRES,
                             "A ping response message was received from Peer (" + makeColComp(Colours.GREEN,
                                                                                              str(senderPeerID)) + ")")
        except socket.error:
            pass
        ...
        try:
            ...
            while True:
                ...
                if msgType == PEERCHURN.QUIT:
                    ...
                    # Print churn message
                    consolePrint(screen, CONTROL.PEERCHURN, "Peer (" + makeColComp(Colours.GREEN, str(
                        senderPeerID)) + ") will depart from the network.")
                    consolePrint(screen, CONTROL.PEERCHURN,
                                 "My first successor is now Peer (" + makeColComp(Colours.GREEN, str(succ1)) + ").")
                    consolePrint(screen, CONTROL.PEERCHURN,
                                 "My second successor is now Peer (" + makeColComp(Colours.GREEN, str(succ2)) + ").")
                ...
                # A peer has responded with query information
                    ...
                    if msgType == FT.FORWARDNEXT:
                        ...
                        consolePrint(screen, CONTROL.FTRES, "File " + makeColComp(Colours.RED, str(filehash).zfill(
                            4)) + " is stored here. A response message has been sent to Peer (" + makeColComp(
                            Colours.GREEN, str(senderPeerID)) + ").")

                    # We received a response for a requested file request
                    elif msgType == FT.RES:
                        consolePrint(screen, CONTROL.FTRES,
                                     "Received a response message from Peer (" + makeColComp(Colours.GREEN, str(
                                         senderPeerID)) + "), which has the file " + makeColComp(Colours.RED,
                                                                                                 str(filehash).zfill(
                                                                                                     4)) + ".")
                    ...
                    # Else perform regular processing
                        ...
                        if fileStatus == FILECHECK.NOTAVAILABLE:
                            ...
                            consolePrint(screen, CONTROL.FTREQ, "File " + makeColComp(Colours.RED, str(filehash).zfill(
                                4)) + " is not stored here. File request message has been forwarded to successor Peer (" + makeColComp(
                                Colours.GREEN, str(succ1)) + ").")
