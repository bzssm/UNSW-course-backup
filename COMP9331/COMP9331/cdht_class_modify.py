...
class CDHT:
    def __init__(self, currentPeer, succ1, succ2):
        ...
        self.name = currentPeer
        self.succ1 = succ1
        self.succ2 = succ2
        self.pred1 = -1  # -1 means not exist
        self.pred2 = -1
    ...
    def ping(self):
        ...
        sock.bind((LOCALHOST, self._peer2port(self.name)))
        ...
        while True:
            if time.time() - lastPingTime > 2.0:  # set ping interval（ping frequency)
                if self.succ1 != 'Dead':
                    self.sendPingMessage(seq, self.name, LOCALHOST, self._peer2port(self.succ1), 'request')
                if self.succ2 != 'Dead':
                    self.sendPingMessage(seq, self.name, LOCALHOST, self._peer2port(self.succ2), 'request')

                lastPingTime = time.time()
                seq += 1

            if succ1Dead and self.succ1 != 'Dead':
                succ1Ack, succ1Dead = seq, False
            if succ2Dead and self.succ2 != 'Dead':
                succ2Ack, succ2Dead = seq, False
            ...
            if self.succ1 != 'Dead' and seq - succ1Ack >= 5:  # set how many missed ack can decide a dead peer
                ...
                deadPeer, self.succ1, succ1Dead = self.succ1, 'Dead', True
                self.sendTCPMessage('prompt', self.name, LOCALHOST, self._peer2port(self.succ2), 'querySucc',
                                    succ1=0, succ2=0)
            if self.succ2 != 'Dead' and seq - succ2Ack >= 5:  # set how many missed ack can decide a dead peer
                ...
                deadPeer, self.succ2, succ2Dead = self.succ2, 'Dead', True
                self.sendTCPMessage('prompt', self.name, LOCALHOST, self._peer2port(self.succ1), 'querySucc',
                                    succ1=0, succ2=0)
            ...
            try:
                ...
                if messageType == 'request':
                    ...
                    if all([self.pred1 != -1, self.pred2 != -1, sender != self.pred1, sender != self.pred2]):
                        self.pred1, self.pred2 = -1, -1
                    if self.pred1 == -1:
                        self.pred1 = sender
                    elif self.pred2 == -1:
                        self.pred2 = sender if sender != self.pred1 else -1

                    self.sendPingMessage(seqRecv, self.name, LOCALHOST, self._peer2port(sender), 'response')
                ...
                while True:
                    ...
                    if message.split(',')[0] == 'prompt':
                        if message.split(',')[1] == 'quit':
                            ...
                            self.sendTCPMessage('prompt', self.name, LOCALHOST, self._peer2port(sender),
                                                'querySuccResponse',
                        ...
                        if message.split(',')[1] == 'Next':
                            self.sendTCPMessage('file', self.name, LOCALHOST,
                                                self._peer2port(message.split(",")[4]), 'Here',
                            ...
                            fileP = self.filePosition(message.split(",")[3], self.name, self.succ1)
                            if fileP == 'Here':
                                self.sendTCPMessage('file', self.name, LOCALHOST,
                                                    self._peer2port(message.split(',')[4]), fileP,
                                                    fileName=message.split(',')[3], originalPeer=message.split(',')[4])

                                print(f'File {message.split(",")[3]} is here.')
                                continue
                            elif fileP == 'Next':
                                self.sendTCPMessage('file', self.name, LOCALHOST, self._peer2port(self.succ1),
                                                    fileP,
                                                    fileName=message.split(',')[3], originalPeer=message.split(',')[4])
                        ...
                        if message.split(',')[1] == 'quit':
                            print(f'Peer {self.name} will depart from the network.')
                            self.sendTCPMessage('prompt', self.name, LOCALHOST, self._peer2port(self.pred1),
                                                'quit',
                                                succ1=self.succ1, succ2=self.succ2)
                            self.sendTCPMessage('prompt', self.name, LOCALHOST, self._peer2port(self.pred2),
                                                'quit',
                                                succ1=self.succ1, succ2=self.succ2)
                            ...
                            try:
                                ...
                                print('Wrong file...')
                                continue
                            fileP = self.filePosition(fileName, self.name, self.succ1)
                            if fileP == 'Here':
                                ...
                                self.sendTCPMessage('file', self.name, LOCALHOST, self._peer2port(self.succ1),
                                                    ...
                                                    originalPeer=self.name)
                            elif not fileP:
                                self.sendTCPMessage('file', int(sender), LOCALHOST, self._peer2port(self.succ1),
                                                    'Unknown',
                                                    fileName=fileNameString, originalPeer=self.name)
