...
class Sudoku:
    ...
    def forced(self, status=None, marked_list=None):
        if status == None:
            status = deepcopy(self.originalStatus)
        originalstatus = deepcopy(status)
        matstatus = []
        for i in range(0, 9, 3):
            for j in range(0, 9, 3):
                matstatus.append(list(chain(*np.array(status)[i:i + 3, j:j + 3].tolist())))
        if marked_list == None:
            marked_list=[]
            for i in range(len(status)):
                for j in range(len(status)):
                    ...
                    for e in np.transpose(np.array(status)).tolist()[j]:
                        used_set.add(e)
                    for e in matstatus[self._findbox(i, j)]:
                        used_set.add(e)
                    if status[i][j] == '0':
        ...
        for i in range(0, 9, 3):
            for j in range(0, 9, 3):
                matMarkedList.append(list(chain(*np.array(marked_list)[i:i + 3, j:j + 3].tolist())))


        positions = []
        for box in matMarkedList:
            print(box)
            allMarkedValue = chain(*box)
            allPossibleValue = [e for e in allMarkedValue if len(e)==1]
            print(allPossibleValue)
            freq = Counter(allPossibleValue)
            print(dict(freq))
            if 1 in freq.values():
                for key in freq:
                    if freq[key]==1:
                        for i in range(9):
                            if key in box[i]:
                                boxNum = matMarkedList.index(box)
                                cellNum = i
                                positions.append(((boxNum // 3) * 3 + cellNum // 3,(boxNum % 3) * 3 + cellNum % 3))
                                status[(boxNum // 3) * 3 + cellNum // 3][(boxNum % 3) * 3 + cellNum % 3]=key
    ...
    def worked_tex_output(self):
        ...
        while True:
            ...
            if matrix == beforematrix and rowmark == beforemarked:
                modifyedmatrix, changedpositions = self.forced(matrix, rowmark)
                if changedpositions != []:
