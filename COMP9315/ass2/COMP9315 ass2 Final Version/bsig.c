...
void findPagesUsingBitSlices(Query q)
{
	...
	Bits querytsig = makeTupleSigWithMK(q->rel, q->qstring, psigBits(q->rel), codeBits(q->rel));
	for (int i=0;i<psigBits(q->rel); i++){
		if (bitIsSet(querytsig, i)){
			Page bsPage = getPage(bsigFile(q->rel), iceil(i+1,maxBsigsPP(q->rel))-1);
			// bsig read
			Bits bs = newBits(bsigBits(q->rel));
			q->nsigs++;

			// bsig pages read record
			if (!bitIsSet(bsigPagesRecord, iceil(i+1,maxBsigsPP(q->rel))-1)){
